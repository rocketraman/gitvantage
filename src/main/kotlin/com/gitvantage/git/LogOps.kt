...
import com.gitvantage.git.model.CommitParent
...
object LogOps {
     ...
     */
    internal fun parseRefs(decoration: String): List<CommitRef> =
        decoration.split(',').map { it.trim() }.filter { it.isNotEmpty() }.flatMap { raw ->
            when {
                raw.startsWith("HEAD ->") ->
                    listOf(CommitRef("HEAD", RefKind.HEAD)) + parseRefs(raw.substringAfter("->").trim())
                raw == "HEAD" -> listOf(CommitRef("HEAD", RefKind.HEAD))
                raw.startsWith("tag:") ->
                    listOf(CommitRef(raw.removePrefix("tag:").trim().removePrefix("refs/tags/"), RefKind.TAG))
                raw.startsWith("refs/tags/") -> listOf(CommitRef(raw.removePrefix("refs/tags/"), RefKind.TAG))
                raw.startsWith("refs/heads/") -> listOf(CommitRef(raw.removePrefix("refs/heads/"), RefKind.LOCAL_BRANCH))
